...
const PatientMessenger = ({ navigation }) => {
      ...
      >
        {!isMe && (
          <View className=" mr-3 items-center justify-center">
            <Image
              source={item.avatar}
              className="w-[30px] h-[30px] rounded-full border border-gray-400"
            />
          </View>
        )}
        ...
        <View className={`max-w-[75%] ${isMe ? "items-end" : "items-start"}`}>
          <View
            className={`px-4 py-3 rounded-2xl ${
              isMe ? "bg-blue-500 rounded-br-md" : "bg-gray-200 rounded-bl-md"
            }`}
          >
            <Text
  ...
  const TypingIndicator = () => (
    <View className="flex-row justify-start mb-4">
      <View className=" mr-3 items-center justify-center">
         <Image
            className="w-[30px] h-[30px] border border-gray-600 rounded-full"
    ...
    <View className="flex-1 bg-background" style={{ paddingTop: insets.top }}>
      ...
      >
        ...
        <View
          ...
          style={{ 
            paddingBottom: Math.max(insets.bottom, 12) + (Platform.OS === 'android' ? keyboardHeight > 0 ? 0 : 0 : 0)
          }}
        >
          <TouchableOpacity
            className="mr-3 p-2"
